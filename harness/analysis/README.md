...
| `lib/io.py` | JSON read/write + the per-frame filename contract (`read_json`, `write_json`, `frame_stem`, `metrics_json`/`depth_json`/`critic_json`) |
...
| `scorers/critic/` | independent **VLM** realism + identity judge (source vs match + turntable); scores + tagged shape/pose/joint fixes — a reliable independent read beside the IoU gate + the noisy depth guide | [critic.md](scorers/critic/critic.md) |
...
| `frames.py` | order frame ids temporally by numeric index (`order_frames`), and resolve a critic `--frames` window around an `--anchor` by offset (`resolve_window`); reads a run's `layout.json` directly (`--run-dir`) so it needs no hand-typed paths, and `window --emit args` prints the exact `--frames …` args a frame-window consumer needs | [frames.md](frames.md) |
